...
import { Plus, Save, X, MoreVertical, Loader2, Sparkles, Eye, Trash2, CheckCircle2, Circle, ClipboardCopy, ScanLine } from "lucide-react";
...
interface ListCardProps {
  ...
  onViewScan?: (imageUrls: string[]) => void; // Changed to accept array
  onDeleteCompletedItemsRequested: (listId: string) => void;
  onScanMoreItemsRequested: (listId: string, listTitle: string) => void;
}

const ListCard: FC<ListCardProps> = ({ list, onUpdateList, onDeleteListRequested, onManageSubitems, startInEditMode = false, onInitialEditDone, toast, onViewScan, onDeleteCompletedItemsRequested, onScanMoreItemsRequested }) => {
  ...
  const [autogeneratePromptValue, setAutogeneratePromptValue] = useState("");
  ...
  const openAutogenerateDialog = () => {
    setAutogeneratePromptValue(list.title);
    setIsAutogenerateDialogOpen(true);
  };

  const handleConfirmAutogenerate = async () => {
    if (!autogeneratePromptValue.trim()) {
      toast({
        title: "Prompt Required",
        description: "Please enter a prompt for item generation.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const input: GenerateSubitemsInput = {
        promptForGeneration: autogeneratePromptValue,
      ...
      if (result && result.newSubitemTitles && result.newSubitemTitles.length > 0) {
        ...
        // No toast for this case, as per user request
      } else {
        toast({
    ...
    <>
      <Card
        className={cn(
          ...
          startInEditMode && !list.completed ? "" : (startInEditMode ? "" : "transition-all duration-300 ease-in-out") // Removed animation, keep transition for completion
        )}
      >
            ...
            {isEditing ? (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  {(list.scanImageUrls && list.scanImageUrls.length > 0 || !list.completed) && <DropdownMenuSeparator />}
                  <DropdownMenuItem
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="autogen-prompt" className="text-right">
                Prompt
              </Label>
              <Input
                id="autogen-prompt"
                value={autogeneratePromptValue}
                onChange={(e) => setAutogeneratePromptValue(e.target.value)}
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={handleConfirmAutogenerate} disabled={isGeneratingItems}>
              {isGeneratingItems ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
              Generate
            </Button>
          </DialogFooter>
        </DialogContent>
